SpectralWorkbench.Image = Class.extend({
    ...
    init: function (_graph, options) {
        ...
        if (image.obj.width > 640) {
            image.width = 640;
        } else {
            image.width = image.obj.width;
        }

        if (image.obj.height > 150) {
            image.height = 150;
        } else {
        ...
        image.getLine = function (y) {
            ...
            for (var i = 0; i < input.length; i += 4) {
                //console.log("output: "+input[i] +" "+input[i + 1]+" "+input[i + 2]+" "+input[i + 3]);
                output.push([input[i],
                    input[i + 1],
                    input[i + 2],
                    input[i + 3]]);
        ...
        image.updateSize = function () {
            ...
            if (!_graph.embed)
                image.container.css('margin-left', _graph.margin.left);
            else
                image.container.css('margin-left', _graph.margin.left);

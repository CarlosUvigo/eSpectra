SpectralWorkbench.Graph = Class.extend({
    ...
    init: function (args, callback) {
        ...
        _graph.onImageComplete = args['onImageComplete'] || function () {
            console.log('image load complete');
        };
        _graph.onComplete = args['onComplete'] || function () {
        ...
        _graph.setUnits = function () {
            ...
            if (!_graph.datum) {
                ...
                if (_graph.args.calibrated)
                    _graph.xUnit = 'nanómetros';
                else
                    _graph.xUnit = "nanómetros";
        ...
        _graph.load = function (datum) {
            ...
            if (_graph.dataType == "spectrum") {
                ...
                if (_graph.datum)
                    _graph.UI = new SpectralWorkbench.UI.Set(_graph, _graph.args.set_id, _graph.datum.spectra);
        ...
        if (_graph.embed)
            $(_graph.selector).on('resize', _graph.updateSize()); // if embed, update the chart when DOM element resizes
        else
            $(window).on('resize', _graph.updateSize()); // else resize when window resizes
    ...
    updateSize: function (newWidth) {
        ...
        return (function () {
            ...
            if (_graph.datum && _graph.datum.image)
                _graph.datum.image.updateSize(); // adjust image element and image.container element
